...
{
  const AVCodec *codec_const = avcodec_find_decoder(AV_CODEC_ID_H264);
  codec = const_cast<AVCodec *>(codec_const);
  if (!codec)
...
#if defined(AV_CODEC_CAP_TRUNCATED) && defined(AV_CODEC_FLAG_TRUNCATED)
...
ssize_t H264Decoder::parse_data(const ubyte *in_data, ssize_t in_size)
{
...
const AVFrame &ConverterRGB24::convert(const AVFrame &frame, ubyte *out_rgb)
{
  ...
  av_image_fill_arrays(framergb->data, framergb->linesize,
                       out_rgb, AV_PIX_FMT_RGB24, w, h, 1);
